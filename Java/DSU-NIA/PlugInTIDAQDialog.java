...
public class PlugInTIDAQDialog extends JDialogStandalonePlugin{
    ...
    public PlugInTIDAQDialog(boolean modal) {
    	super(modal);
    	javax.swing.SwingUtilities.invokeLater(new Runnable() {
    		public void run() { 
    			init();
    ...
    public PlugInTIDAQDialog(Dialog parent, boolean modal) {
    	super(parent, modal);
    	javax.swing.SwingUtilities.invokeLater(new Runnable() {
    		public void run() { 
    			init();
    ...
    public PlugInTIDAQDialog(Frame parentFrame, boolean modal) {
    ...
    void init() {
        ...
        setTitle("Tissue Composition Assessment from (pQ)CT - DSU and NIA (by Sokratis Makrogiannis, PhD)");
...
void addLogoPanel() {
	...
	// Add logo script.
	JLabel logoScript = new JLabel("<HTML>TIDAQ (Tissue Identification and Quantification) v.1.4 <BR> DSU and NIA </HTML> <BR>");
...
//		PlugInPQCTAnalysisMIPAV.class.getResource("images/NIA_logo.png");
	...
	String imagePath = currentDirectory +
		...
		File.separator + "DSU_NIA_logo.png";
...
//			logoImage = ImageIO.read(new File(imageURL.getPath()));
		...
		g2d.drawImage(logoImage, 2, 2, 256, 128, this);
	}
